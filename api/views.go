...
func RenderIndex(ctx echo.Context) error {
	return utils.Render(ctx, http.StatusOK, pages.Home())
}

func RenderLogin(ctx echo.Context) error {
	return utils.Render(ctx, http.StatusOK, pages.Login())
}

func RenderRegister(ctx echo.Context) error {
	return utils.Render(ctx, http.StatusOK, pages.Register())
}
...
func (s Server) RenderHistory(ctx echo.Context) error {
	...
	if userCtx.IsLoggedIn {
		...
		if err != nil {
			return utils.Render(ctx, http.StatusOK, pages.History(websites))
		}
